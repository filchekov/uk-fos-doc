...
(function($) {
	$().ready(function() {
					...
					parents.each(function() {
						$(this).on('change keyup', function() {
							var value		= $(this).is(':checkbox') ? $(this).prop('checked') : $(this).val(),
								filled		= value && typeof value !== 'undefined',
								match_found = false;
		...
		function ukFosSendMail(form, settings) {
			...
			var honeypot = form.find('[name="uk_fos_proof"]').val();
			if (typeof honeypot === 'undefined') honeypot = '';

			var extra_inputs = form.find('[data-added]');

			if (extra_inputs) { // TODO: Is there needed a "lenght > 0" check?
				...
				added.attr('value', JSON.stringify(added_values)); // TODO: Check this
			}
			...
			if (ukFosModernUploadSupport.api) {
				...
				formdata.append('module', 'uk_fos');
				formdata.append('format', 'raw');
				formdata.append('Itemid', settings.Itemid);
				formdata.append('mod_id', settings.mod_id);
				formdata.append('uk_fos_proof', honeypot);

				if (settings.track_url) {
					var origin = form.find('[name="uk_fos_origin"]').val();
					formdata.append('uk_fos_origin', origin);
				}
